...
public interface Either<L, R> {
	...
	class LeftProjection<L, R> extends Projection<L, R>{
		...
		public <Next> LeftProjection<Next, R> map(Function<L, Next> func){
			if(either.isLeft()){
		...
		@SuppressWarnings("unchecked")
		public <Next> LeftProjection<Next, R> flatMap(Function<L, Either<Next, R>> func){
			if(either.isLeft()){
		...
		public Option<L> filter(Function<L, Boolean> func) {
		...
		public void forEach(Function<L, ?> func) {
			if(either.isLeft())
				func.apply(asLeft());
		}
	...
	class RightProjection<L, R> extends Projection<L, R>{
		...
		@SuppressWarnings("unchecked")
		public <Next> RightProjection<L, Next> map(Function<R, Next> func){
			if(either.isRight()){
		...
		@SuppressWarnings("unchecked")
		public <Next> RightProjection<L, Next> flatMap(Function<R, Either<L, Next>> func){
			if(either.isRight()){
		...
		public Option<R> filter(Function<R, Boolean> func) {
		...
		public void forEach(Function<R, ?> func) {
			if(either.isRight())
				func.apply(asRight());
		}
